...
class MapBlocks
{
    ...
    public:
        ...
        static int BLOCK_ABS_X;
        static int BLOCK_ABS_Y;

        // coordinates of each block relative to camera
        static int BLOCK_REL_X;
        static int BLOCK_REL_Y;

        static int BLOCK_SPRITE; // Map to which sprite image this block will use.
...
public:
    ...
    void moveBlocks()
    {
        ...
        for(i = 0; i < BLOCKS_N; i++)
        {
            blocks_arr[i].BLOCK_REL_X = blocks_arr[i].BLOCK_ABS_X;
            blocks_arr[i].BLOCK_REL_X = blocks_arr[i].BLOCK_ABS_X;
        }
    }
